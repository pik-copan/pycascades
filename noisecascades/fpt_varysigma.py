...
def add_FPT_to_cube(zarrpath = "varysigma_simulations.zarr"):
    ...
    newds = zarrgroup.create_dataset("FPT", shape = (len(cube.config_id), len(cube.GMT), len(cube.strength), len(cube.alpha), len(cube.sigma), len(cube.seed)), chunks = (1, 1, 1, 1, 1, len(cube.seed)), dtype = 'float32', fillvalue = np.nan, compressor = compressor)
    newds.attrs['_ARRAY_DIMENSIONS'] = ("config_id", "GMT", "strength", "alpha", "sigma", "seed")

    zarrgroup["FPT"][:] = FPT.FPT.values

    newds = zarrgroup.create_dataset("Oktant", shape = (len(cube.config_id), len(cube.GMT), len(cube.strength), len(cube.alpha), len(cube.sigma), len(cube.seed)), chunks = (1, 1, 1, 1, 1, len(cube.seed)), dtype = '<U3', fillvalue = '000', compressor = compressor)
    newds.attrs['_ARRAY_DIMENSIONS'] = ("config_id", "GMT", "strength", "alpha", "sigma", "seed")
