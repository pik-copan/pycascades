...
def simulate_network_config_from_vars(vars):
    ...
    for seed in range(n_runs):
        dss.append(simulate_network_config(config, seed, GMT, strength, sigma_as_coord=True))
    ds = xr.merge(dss)
    zarrpath = "varysigma_simulations.zarr"
    zarrgroup = zarr.open_group(str(zarrpath))
    for var in ["GIS", "THC", "WAIS", "AMAZ"]:
        zarrgroup[var][:, config_idx, GMT_idx, strength_idx, alpha_idx, sigma_idx, :] = ds[var].values[:,0,0,0,0,0,:]
    
def simulate_all_for_varysigma(sigmas = [0.01, 0.1, 1.0, 10.0], n_runs = 20, configs_file = "network_configs.csv", timestep = 10., t_end = 100000):
    ...
    for var in ["GIS", "THC", "WAIS", "AMAZ"]:
        newds = zarrgroup.create_dataset(var, shape = (int(t_end // timestep) + 1, len(df.config_id.unique()), len(df.GMT.unique()), len(df.strength.unique()), len(df.alpha.unique()), len(sigmas), n_runs), chunks = (int(t_end // timestep) + 1, 1, 1, 1, 1, 1, n_runs), dtype = 'float32', fillvalue = np.nan, compressor = compressor)
