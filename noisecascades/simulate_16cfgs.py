...
def simulate_network_config_from_vars(vars):
    config, GMT, strength, alpha_idx, config_idx, GMT_idx, strength_idx, sigma_idx, n_runs = vars
    n_batches = n_runs // 100
    for batch in range(n_batches):
        dss = []
        for seed in range(100):
            dss.append(simulate_network_config(config, batch*100 + seed, GMT, strength, sigma_as_coord=True))
        ds = xr.merge(dss)
        zarrpath = "cfgs16_simulations.zarr"
        zarrgroup = zarr.open_group(str(zarrpath))
        for var in ["GIS", "THC", "WAIS", "AMAZ"]:
            zarrgroup[var][:, config_idx, GMT_idx, strength_idx, alpha_idx, sigma_idx, batch*100:(batch+1)*100] = ds[var].values[:,0,0,0,0,0,:]
    
def simulate_all_for_16cfgs(n_runs = 10000, configs_file = "network_configs.csv", timestep = 10., t_end = 100000):
            ...
            for config_id in coords["config_id"]:
                for strength in coords["strength"]:
                    for GMT in coords["GMT"]:
                        ...
                        vars.append((config, GMT, strength, alpha_idx, config_idx, GMT_idx, strength_idx, sigma_idx, n_runs))

    print("building dummy")
    ds = xr.Dataset(coords = coords)
    
    ds = ds.chunk(chunks={"time": -1, "config_id": 1, "seed": -1, "GMT": 1, "strength": 1, "alpha": 1, "sigma": 1})
    ...
    for var in ["GIS", "THC", "WAIS", "AMAZ"]:
        newds = zarrgroup.create_dataset(var, shape = (int(t_end // timestep) + 1, len(coords["config_id"]), len(coords["GMT"]), len(coords["strength"]), len(coords["alpha"]), len(coords["sigma"]), n_runs), chunks = (int(t_end // timestep) + 1, 1, 1, 1, 1, 1, n_runs), dtype = 'float32', fillvalue = np.nan, compressor = compressor)
        newds.attrs['_ARRAY_DIMENSIONS'] = ("time", "config_id", "GMT", "strength", "alpha", "sigma", "seed")
