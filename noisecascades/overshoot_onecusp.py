...
@jit(nopython = True)
def overshoot(x_init:float = 0.0, alpha:float = 2.0, tao:float = 1700.0, gamma:float = 1.0, N: int = int(1e7)):
...
def est_overshoot(x_init:float = 0.0, alpha:float = 2.0, tao:float = 1700.0, gamma:float = 1.0, N: int = int(1e7), M: int = 100):
    idxs = np.zeros(M)
    for i in tqdm(range(M)):
        idxs[i] = overshoot(x_init = x_init, alpha = alpha, tao = tao, gamma = gamma, N = N)
    ...
    return np.nanmean(idxs)


def main():
    N = int(1e7)
    M = 100
    outdict = {"Element": [], "X0": [], "Tao": [], "Alpha": [], "Gamma": [], "P(T-1 < T+1 | Tx)": []}
        ...
        for j, tao in enumerate([1865.4339212188884, 114.21024007462583, 913.6819205970066, 19.035040012437637]):
            ...
            for alpha in [0.5, 1.0, 1.5, 2.0]:#np.linspace(0.25, 2.0, 8):#np.linspace(0.1, 2.0, 20):#
                for gamma in [0.1, 1.0, 10.0]:#[0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 50.0, 100.0]:#
                    print(f"Recovery proba for x0 = {x_init}, tao = {tao}, alpha = {alpha}, gamma = {gamma}")
                    proba = est_overshoot(x_init = x_init, alpha = alpha, tao = tao, gamma = gamma, N = N, M = M)
                    ...
                    outdict["P(T-1 < T+1 | Tx)"].append(proba)

    df = pd.DataFrame.from_dict(outdict)

    df.to_csv(f"overshoot_onecusp.csv{M}")

    #df = pd.read_csv(f"overshoot_onecusp.csv{M}")

    g = sb.FacetGrid(df, row="Element",  col = "X0")
    g.map_dataframe(lambda data, color: sb.heatmap(data.pivot("Alpha", "Gamma", "P(T-1 < T+1 | Tx)"), square=True, vmin = 0.0, vmax = 1.0, cbar = False))
    cbar_ax = g.fig.add_axes([1.015,0.13, 0.015, 0.8])
    plt.colorbar(mpl.cm.ScalarMappable(norm=mpl.colors.Normalize(vmin=0.0, vmax=1.0), cmap = sb.color_palette("rocket", as_cmap=True)),cax=cbar_ax, label = "P(T-1 < T+1 | Tx)")
    g.fig.savefig(f"overshoot_onecusp_RecoveryProba.png", dpi = 200, facecolor='white', transparent=False, bbox_inches="tight")
    plt.clf()
