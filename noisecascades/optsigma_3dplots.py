...
cube = xr.open_zarr( "optsigma_simulations.zarr", consolidated = False)
...
GMT_init = 0.0
tend_init = 10000
traj = cube.sel(config_id = 0, GMT= GMT_init, seed = seed_init, strength = strength_init, alpha = alpha_init, time = slice(tend_init))
row = df[(df.config_id == 0) & (df.GMT == GMT_init) & (df.strength == strength_init) & (df.alpha == alpha_init)].iloc[0 if alpha_init != 2.0 else 1]
...
def update(val):
    traj = cube.sel(config_id = 0, GMT= s_GMT.val, seed = s_seed.val, strength = s_strength.val, alpha = s_alpha.val, time = slice(s_tend.val))
    row = df[(df.config_id == 0) & (df.GMT == s_GMT.val) & (df.strength == s_strength.val) & (df.alpha == s_alpha.val)].iloc[0 if alpha_init != s_alpha.val else 1]
