...
def main():
    ...
    df.to_csv("first_passage_simple.csv")


    for var in ["Mean", "Median"]:
        g = sb.FacetGrid(df, col="Element",  col_wrap = 2)
        g.map_dataframe(lambda data, color: sb.heatmap(data.pivot("Alpha", "Gamma", "Mean"), square=True, norm=LogNorm(vmin = 1.0, vmax = 1e7), cbar = False))
